...
// MonthMetada 생성
try monthMetadata(for: Date())

generateDaysInMonth(for: Date())

generateDay(offsetBy: -1, for: Date(), isWithinDisplayedMonth: true)

generateStartOfNextMonth(using: Date(timeIntervalSince1970: 1675296000))
...
let endOfJanuary = Date(timeIntervalSince1970: 1675123200)
calendar.date(byAdding: .month, value: 1, to: endOfJanuary)
calendar.date(byAdding: .month, value: 2, to: endOfJanuary)
calendar.date(byAdding: .month, value: 3, to: endOfJanuary)
calendar.date(byAdding: .month, value: 4, to: endOfJanuary)
